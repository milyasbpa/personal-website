...
3. **No purple or indigo** — The brand color is **GreatFrontEnd Emerald Green** (#059669 light / #34D399 dark). Purple, violet, and indigo are forbidden.
...
6. **TypeScript strict mode** — No `any` types. Define proper interfaces and types in `src/types/`.
...
9. **Every component needs a Storybook story** — All components in `src/components/` must have a corresponding `.stories.tsx` file.
10. **Write tests for utilities and hooks** — All files in `src/lib/` and `src/hooks/` must have test coverage.
...
- Types: `camelCase.ts` in `src/types/` (e.g., `blog.ts`, `learning.ts`)
